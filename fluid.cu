...
void render_fluid(uint8_t *render_target, dims img_dims, float *volume /*pointer to gpu mem?*/, dims vol_dims, fdims cam_pos, float rotation);

void save_image(uint8_t *pixels, dims img_dims, std::string name) {
    std::ofstream file(name, std::ofstream::binary);
    if (file.is_open()) {
        file << "P6\n" << img_dims.x << " " << img_dims.y << "\n" << "255\n";
        file.write(pixels, img_dims.x*img_dims.y*3);
        file.close();
    } else {
