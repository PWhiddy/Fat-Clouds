	...
	fdims light_dir, fdims cam_pos, float rotation)
{
	...
	float3 pos = make_float3(cam_pos.x, cam_pos.y, cam_pos.z);
	...
	image[pixel+0] = (uint8_t)(200.0*ray_dir.x);
	image[pixel+1] = (uint8_t)(accum);
	image[pixel+2] = (uint8_t)(200.0*ray_dir.z);
}
